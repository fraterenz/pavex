...
fn process_error_observer(
    ...
    let id = intern_component(
        ...
        component,
        LIFECYCLE,
        eo.error_observer.location.clone(),
    );
...
fn process_prebuilt_type(aux: &mut AuxiliaryData, si: &PrebuiltType, current_scope_id: ScopeId) {
    ...
    let id = intern_component(aux, component, LIFECYCLE, si.input.location.clone());
    aux.id2cloning_strategy.insert(
        id,
...
fn process_config_type(aux: &mut AuxiliaryData, t: &ConfigType, current_scope_id: ScopeId) {
    ...
    let id = intern_component(aux, component, LIFECYCLE, t.input.location.clone());
    aux.id2cloning_strategy.insert(
        id,
        t.cloning_strategy
...
fn process_error_handler(
    ...
    intern_component(aux, component, lifecycle, error_handler.location.to_owned());
}
...
mod diagnostics {
    ...
    pub(super) fn route_path_must_start_with_a_slash(
        ...
        let source = diagnostics.source(location).map(|s| {
            diagnostic::get_route_path_span(s.source(), location)
    ...
    pub(super) fn invalid_domain_guard(
        ...
        let source = diagnostics.source(&location).map(|s| {
            diagnostic::get_domain_span(s.source(), location)
    ...
    pub(super) fn path_prefix_cannot_be_empty(
        ...
        let source = diagnostics.source(&location).map(|s| {
            diagnostic::get_prefix_span(s.source(), location)
